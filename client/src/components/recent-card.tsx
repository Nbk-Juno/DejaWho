...
export function RecentCard({ name, location, date, isFresh, onClick }: RecentCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      className={[
        "flex-shrink-0 w-[180px] text-left px-3.5 py-3 rounded-2xl bg-white/[0.04] border border-white/10 hover:bg-white/[0.07] active:bg-white/[0.09] transition-colors flex flex-col gap-1.5",
        isFresh ? "dw-recent-fresh" : "",
      ].join(" ")}
    >
