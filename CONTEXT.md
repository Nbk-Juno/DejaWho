...
The module (`server/person-clustering.ts`) that owns the [[Person]] lifecycle from ADR-0001: deciding which Person a freshly-written [[Encounter]] belongs to (`resolveAndAttach`), keeping affected Person rows consistent (`recompute` — recount, retag to the latest encounter, keep the latest known last name, null the cached summary, drop the row when empty), reconciling both sides of a merge (`reassign`) or a rename (`reclusterAfterRename`). It composes the pure `resolvePerson` scorer with the storage CRUD seam and is AI-free, so it's unit-testable against any `IStorage` adapter. Built via `createPersonClustering(storage)`; the app uses the `personClustering` singleton, tests construct one over `MemStorage`. Summary *invalidation* lives here; summary *generation* stays in the `GET /api/persons/:id` route (it needs billing + OpenAI). This is the natural home for the eventual B-full work (fuzzy matching, merge/split) the ADR defers.
