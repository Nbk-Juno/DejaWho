...
E2E tests in `./e2e/` use stable `data-testid` attributes as their contract with the UI. If you remove or rename a testID that e2e relies on, the e2e job will fail and block the deploy. Current testIDs the suite depends on: `input-email`, `input-password`, `button-sign-in`, `button-google-signin`, `auth-error`, `home-loaded`, `button-sign-out`, the landing testIDs (`landing-loaded`, `nav-sign-in`, `waitlist-email-hero`, `waitlist-submit-hero`), `voice-button` (home Voice Button), `mode-toggle` (record/ask switch), and `search-voice-button` (Search page mic) — all used by the voice-loop e2e — the delete-person testIDs `person-row-<id>` / `button-delete-person-<id>` / `button-confirm-delete-person` (Search page, used by `delete-person.test.ts`), plus the reset-password page testIDs.
...
**Scope: front-door smoke + the voice loop.** Beyond auth wiring, sign-in/out, and shell/`/search` render, `e2e/voice-loop.test.ts` now exercises the core voice loop end-to-end: it installs a fake `MediaRecorder` + `getUserMedia` via `addInitScript` and intercepts the AI endpoints (`/api/transcribe`, `/api/parse-encounter`, `/api/encounters`, `/api/search`) with `page.route` so the run is deterministic and OpenAI-free. It asserts: home Voice Button (record mode) → transcribes → saves (the "Saved!" toast); home Voice Button switched to search mode via the `mode-toggle` → transcribes → searches → sends the answer to TTS (asserted by observing the `/api/text-to-speech` request, which proves `useVoiceResponse` ran); and the Search page mic → transcribes → shows a result. The client save/search wiring is also unit-tested in node via `tests/save-encounter.test.ts`. The **delete flows** are covered too: `tests/person-resolve-api.test.ts` exercises both `DELETE` route handlers over HTTP — encounter delete (drop-the-now-empty-person vs. recompute-the-count) and person delete (cascade), plus 404s and per-user scoping — and `e2e/delete-person.test.ts` drives the Search-page delete-person UI (trash button → confirm dialog → `DELETE` → list refetch). Still not covered: the delete-**encounter** UI (the encounter-detail sheet's `button-delete-encounter-detail` → `button-confirm-delete`; its route handler *is* HTTP-tested), and the swipe gesture itself (the toggle is used to switch modes instead).
...
Supabase auth: **email/password** (primary — solves iOS PWA session isolation), **Google SSO** (`signInWithGoogle` → `supabase.auth.signInWithOAuth`, the "Continue with Google" button), and **password reset**. (Magic-link was removed — unfamiliar in consumer-mobile and broken in the installed iOS PWA.) All issue JWTs verified identically by `requireAuth`. Google SSO requires the Google provider enabled in **both** Supabase projects and the app origins (`https://dejawho.io/**`, `https://dejawho.onrender.com/**`, `http://localhost:5050/**`) in each project's **Redirect URLs**; the OAuth round-trip returns to the app origin and the session is parsed by `detectSessionInUrl`. A non-allow-listed Google email still hits the invite-only screen via `/api/me`. The OAuth round-trip can't be deterministically e2e'd (only the button's render is asserted) and its iOS-standalone-PWA behavior must be verified on a real device. JWT bearer tokens (no cookies). The server-side middleware `requireAuth` (in `server/auth.ts`) calls `supabase.auth.getUser(token)` to verify the bearer token and attaches `req.user = { id, email }`. Don't apply `requireAuth` (or `userIdFrom`) by hand on new routes — register them through the **Guarded Route** envelope in `server/route.ts` (`get`/`post`/`patch`/`del`), which applies `requireAuth` + the allow-list and hands the handler `{ userId, body }`. `/api/health` and `/api/waitlist` are the unauthenticated carve-outs and stay plain `app.*` routes.

The sign-in page (`client/src/pages/sign-in.tsx`) is a single email/password form (no tabs) with a sign-in/sign-up mode toggle and a "Forgot password?" flow. Invited users arrive via the invite email's deep-link `/sign-in?mode=signup&email=<addr>` (read once from the query string at mount), which opens account-setup mode with the email pre-filled so a brand-new user lands on "create a password" rather than a sign-in form. Account creation calls `supabase.auth.signUp`; the Supabase **"Confirm email" provider setting must be OFF** (the invite email already verifies the address and the allow-list is the real access gate) so signup returns a live session immediately instead of sending a redundant confirmation email. Set it off in both dev and prod projects. Password reset calls `supabase.auth.resetPasswordForEmail()` with a redirect to `/reset-password`. The reset-password page (`client/src/pages/reset-password.tsx`) handles the Supabase callback (recovery token in URL hash, parsed by `detectSessionInUrl: true`) and calls `supabase.auth.updateUser({ password })`. The `/reset-password` route renders outside the auth gate in `App.tsx` so recovery sessions work. Auth methods (`resetPassword`, `updatePassword`) live in `use-auth.tsx`.
...
Production runs on **Render** free tier (auto-deploy from `main`), backed by **Supabase** (Auth + Postgres + RLS). Custom SMTP via **Resend** for Supabase auth emails such as password reset (configured in Supabase Auth settings, not in app code). Live at `https://dejawho.io` (Render also serves the underlying `https://dejawho.onrender.com`).
