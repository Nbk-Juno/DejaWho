...
E2E tests in `./e2e/` use stable `data-testid` attributes as their contract with the UI. If you remove or rename a testID that e2e relies on, the e2e job will fail and block the deploy. Current testIDs the suite depends on: `input-email`, `input-password`, `button-sign-in`, `auth-error`, `home-loaded`, `button-sign-out`, the landing testIDs (`landing-loaded`, `nav-sign-in`, `waitlist-email-hero`, `waitlist-submit-hero`), `voice-button` (home Voice Button), `mode-toggle` (record/ask switch), and `search-voice-button` (Search page mic) — all used by the voice-loop e2e — the delete-person testIDs `person-row-<id>` / `button-delete-person-<id>` / `button-confirm-delete-person` (Search page, used by `delete-person.test.ts`), plus the reset-password page testIDs.
