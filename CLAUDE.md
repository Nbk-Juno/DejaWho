...
E2E tests in `./e2e/` use stable `data-testid` attributes as their contract with the UI. If you remove or rename a testID that e2e relies on, the e2e job will fail and block the deploy. Current testIDs the suite depends on: `tab-password`, `tab-magic-link`, `input-email`, `input-password`, `button-sign-in`, `auth-error`, `input-magic-email`, `button-send-magic-link`, `home-loaded`, `button-sign-out`, the landing testIDs (`landing-loaded`, `nav-sign-in`, `waitlist-email-hero`, `waitlist-submit-hero`), `voice-button` (home Voice Button) and `search-voice-button` (Search page mic, used by the voice-loop e2e), plus the reset-password page testIDs.
...
**Scope: front-door smoke + the voice loop.** Beyond auth wiring, sign-in/out, and shell/`/search` render, `e2e/voice-loop.test.ts` now exercises the core voice loop end-to-end: it installs a fake `MediaRecorder` + `getUserMedia` via `addInitScript` and intercepts the AI endpoints (`/api/transcribe`, `/api/parse-encounter`, `/api/encounters`, `/api/search`) with `page.route` so the run is deterministic and OpenAI-free. It asserts that the home Voice Button records → transcribes → saves (the "Saved!" toast), and that the Search page mic records → transcribes → shows a result. The client save/search wiring is also unit-tested in node via `tests/save-encounter.test.ts`. Still not covered: TTS playback assertions, delete-encounter, and the home **search** mode (reaching it needs the swipe gesture).
